...
The specific problem i encountered that made me in need of this script was porting minecraft structures from my minecraft world onto a friend's world. Structure blocks are local to specific worlds, so this cannot be done without accessing the files of the world. In this situation, my friend played on console and either didn't have the means to or didn't understand how to access these files, meaning that i couldn't send a structure across, or install a behavoir pack on their world.
...
```
<sup><sub>(short: -i)</sub></sup>

This is the interval between each command, and is set to 0.5 seconds by default. You should increse this if lag is causing some commands to be skipped, as the script has no way of knowing when minecraft's chat has fully closed or opened.
